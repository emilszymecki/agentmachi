...
**Interfaces:**
- Produces: wyjątki `TaskError`, `Conflict(TaskError)`, `StaleGeneration(TaskError)`; `TaskQueue(wip_limit=3, lease_ttl=120.0, dedup_ttl=None)` z: `add(card: dict, command_id: str, now: float) -> dict`, `claim(task_id, nick, generation, command_id, expected_version, now) -> dict`, `get(task_id) -> dict`. Task = dict: `{id, card, status, assignee, generation, version, lease_until, frozen}`. Dedup: powtórzone `command_id` zwraca ZAPAMIĘTANY wynik, bez mutacji. CAS: `expected_version != task["version"]` → `Conflict` bez mutacji. Mutacja taska z `generation != task["generation"]` → `StaleGeneration` (dla claim: generation zapisywana, nie sprawdzana).
