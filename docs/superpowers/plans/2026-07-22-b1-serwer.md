...
> UWAGA (po review): sygnatura to TaskQueue(wip_limit=3, lease_ttl=120.0) — dedup_ttl USUNIĘTY decyzją tercetu; wpisy dedup żyją do kontrolowanej kompakcji.
...
## ANEKS v2 (2026-07-22, korekta @Emil: LESS IS MORE — grupy, nie RBAC)

Permission matrix/RBAC USUNIĘTE z zakresu B1 (future consideration).
Role = aliasy grupowe w mechanice wzmianek. Wpływ na plan:

- **Task 6 (server) rozszerzony o grupy**: do `chat/protocol.py` dochodzi
  `parse_groups(text: str) -> set[str]` (regex `(?:^|\s)\$(\w+)`, testy
  analogiczne do `parse_mentions`). Hello niesie `groups: list[str]`
  (z agent card); serwer trzyma `groups: dict[nick, set[str]]`. Routing
  chatu: `$group` rozwija się do AKTUALNYCH członków (poza nadawcą),
  nieznana grupa → ramka `error` do nadawcy i ŻADNEJ publikacji do grupy
  (reszta wzmianek z tej ramki działa normalnie). Hello zwraca też
  `rules` + `rules_hash` (treść pliku `rules.md` obok danych huba, jeśli
  istnieje; hash = sha256). Testy dodatkowe (kontrakt codexa):
  `$workers` budzi dokładnie członków grupy i nikogo więcej; membership
  przeżywa reconnect (drugie hello, te same groups → wzmianka nadal
  dociera); nieznana grupa → jawny error, zero cichego broadcastu.
- **Task 3 (identity) BEZ rozszerzeń RBAC** — zostaje jak w planie
  bazowym (tokens dict, generation/instance). Enrollment-kody grup,
  agent_id i plik 0600 przenoszą się do B2 (skill), bo north-star UX
  ukrywa je w skillu; serwer w B1 potrzebuje tylko tokenów i grup.
- **Task 7**: `tokens.example.json` zostaje; dodatkowo `.gitignore`
  (`__pycache__/`, `chat-data/`, `.chat-session.json`, `.superpowers/`,
  `hub.toml`, `tokens.json`) oraz `rules.example.md` (konstytucja
  przykładowa). Sekret/token NIGDY w argv — `CHAT_TOKEN` przez env.
- Launcher `cowork_agents_4_all` i TUI — B4; reload rules w locie — B2.
