...
> **Polski oryginał README.** Wersja wejściowa projektu jest po angielsku:
> [`README.md`](../../README.md) w korzeniu repo. Ten plik zostaje jako
> zapis po polsku i jest trzymany w zgodzie z tamtym — gdy się rozjadą,
> prawdą jest angielski.
...
i uzasadnienie: [`docs/pl/konstytucja.md`](konstytucja.md).
...
`install-skills` wypakowuje skille z pakietu do katalogów harnessów
(`~/.claude/skills`, `~/.agents/skills`) — bez klonowania repo. TUI
człowieka jest w extras: `pip install 'agentmachi[tui]'` (dociąga
`textual`); reszta chodzi na samym `websockets`.
...
Nie musisz tego pamiętać: po `install-skills` powiedz swojemu Claude Code
albo Codexowi *„odpal pokój dla agentów"*. Warianty instalacji (w tym
symlink z klonu repo, dla pracujących **nad** agentmachi) —
...
w [`docs/pl/superpowers/plans/2026-07-24-plan-wyciecia-obory.md`](superpowers/plans/2026-07-24-plan-wyciecia-obory.md)
(Task C1).
...
każda z dowodem i kosztem: [`docs/pl/zasady-agentyczne.md`](zasady-agentyczne.md).
Konstytucja projektu („łąka, nie obora") — obowiązująca bramka każdej
zmiany: [`docs/pl/konstytucja.md`](konstytucja.md).
Spec i plany: `docs/pl/superpowers/`.
...
tests/                 pytest
docs/philosophy.md     skrót tez po angielsku
docs/pl/               polskie oryginały: konstytucja, zasady, spec + plany
```
