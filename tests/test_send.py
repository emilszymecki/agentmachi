...
def test_send_odmowe_limitera_zglasza_wyjatkiem_a_ramki_nie_ma_w_logu(
        tmp_path, monkeypatch, capsys):
    """CICHY FALSE-SUCCESS, ktory sami wprowadzilismy 2026-08-06.

    Repro (Codex, potwierdzony): hub z kubelkiem 1 B/s i burstem 64 KiB.
    Pierwsza duza ramka wchodzi, druga odbija sie o limit. Wynik przed
    naprawa: w logu JEDNA ramka `chat`, a `agentmachi send` konczy sie
    ZEREM — komunikat leci tylko na stderr. Skrypt czytajacy exit 0 jako
    "wyslano" gubil wiadomosc bez sladu.

    Przyczyna nie byla przypadkiem: `_pokaz_ostrzezenie_serwera` zwracalo
    ramke `error`, a `send_once` ignorowal zwrotke — bo do dnia limitera
    `error` po `chat` znaczylo WYLACZNIE ostrzezenie, przy ktorym zero jest
    poprawne.

    Sprawdzamy OBA fakty naraz, bo osobno kazdy z nich klamie: wolajacy
    dostaje sygnal bledu ORAZ ramki naprawde nie ma na dysku."""
    from chat.server import ChatServer
    from chat import protocol
    port = _free_port()
    monkeypatch.delenv("CHAT_TOKEN", raising=False)
    ...
    monkeypatch.setattr(send, "URI", f"ws://localhost:{port}")
    monkeypatch.setattr(send, "HUB_ID", f"localhost:{port}")
    duza = "x" * 40_000          # dwie takie nie mieszcza sie w burscie
    ...
    async def scenario():
        ...
        await srv.start()
        try:
            # Pierwsza MUSI przejsc — inaczej test przechodzilby takze na
            # hubie, ktory po prostu blokuje wszystko.
            await send.send_once("agent1", "PRZESZLA-" + duza)
            with pytest.raises(send.SessionError) as e:
                await send.send_once("agent1", "ODRZUCONA-" + duza)
            return str(e.value), [f.get("text", "") for f in srv.log.replay()
                                  if f.get("type") == "chat"]
        finally:
            await srv.stop()

    komunikat, teksty = asyncio.run(scenario())
    # Wyjatek jest z rodziny SessionError, bo `cmd_send` lapie wlasnie ja
    # i konczy kodem 1 (patrz test_cli.py).
    assert "rate limit" in komunikat, komunikat
    # Naprawa (ile czekac) ma przezyc droge z serwera do agenta.
    assert "Wait" in komunikat, komunikat
    ...
    assert not any(t.startswith("ODRZUCONA-") for t in teksty), \
        "odrzucona ramka wyladowala w logu — wtedy wyjatek jest klamstwem"


def test_send_ostrzezenie_o_nieznanym_nicku_NIE_jest_odmowa(tmp_path,
                                                            monkeypatch):
    """WARUNEK GRANICZNY naprawy powyzej: ostrzezenie ma dalej konczyc sie
    ZEREM, bo ramka faktycznie wchodzi do logu i dociera do ludzi.

    Bez tego testu naprawa cichego false-successu zamienilaby go po prostu
    na falszywy sygnal w druga strone: skrypt czytajacy niezerowy kod jako
    "nie wyslano" wyrzucalby wiadomosc, ktora doszla. To jest istniejacy,
    celowy kontrakt (`_pokaz_ostrzezenie_serwera`), a nie efekt uboczny.

    `send_once` NIE MOZE tu rzucic — asercja jest w tym, ze scenariusz
    dochodzi do konca (wyjatek wywroci test)."""
    from chat.server import ChatServer
    port = _free_port()
    monkeypatch.delenv("CHAT_TOKEN", raising=False)
    monkeypatch.setenv("CHAT_SESSION_DIR", str(tmp_path / "sess"))
    ...
    async def scenario():
        ...
        try:
            await send.send_once("agent1", "@nikt-takiego halo")
            # --quiet (typ `fyi`) idzie TA SAMA sciezka send_once, wiec musi
            # miec ten sam kontrakt. Osobne wywolanie, bo `quiet` zmienia typ
            # ramki i latwo o rozjazd przy nastepnej zmianie.
            await send.send_once("agent1", "@nikt-takiego cicho", quiet=True)
            return [(f.get("type"), f.get("text", "")) for f in srv.log.replay()]
        finally:
            await srv.stop()

    wpisy = asyncio.run(scenario())
    assert ("chat", "@nikt-takiego halo") in wpisy, wpisy
    assert ("fyi", "@nikt-takiego cicho") in wpisy, wpisy
